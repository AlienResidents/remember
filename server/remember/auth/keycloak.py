...
class KeycloakAuthProvider(AuthProvider):
    ...
    async def authenticate(self, **kwargs) -> AuthResult:
        ...
        Returns:
            AuthResult with Keycloak user information
    ...
    async def _exchange_code(self, code: str) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{self.config.authority_url}/protocol/openid-connect/token",
                data={
    ...
    async def _get_user_info(self, access_token: str) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{self.config.authority_url}/protocol/openid-connect/userinfo",
                headers={
    ...
    async def _get_or_create_user(self, db, user_info: dict) -> User:
        """Get or create a user from Keycloak info."""
        from sqlalchemy import select

        provider_id = str(user_info["sub"])
        stmt = select(User).where(User.provider == "keycloak", User.provider_id == provider_id)
