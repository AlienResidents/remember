...
class AuthentikAuthProvider(AuthProvider):
    ...
    async def _exchange_code(self, code: str) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{self.config.authority}/application/o/token/",
                data={
    ...
    async def _get_user_info(self, access_token: str) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{self.config.authority}/application/o/userinfo/",
                headers={
    ...
    async def _get_or_create_user(self, db, user_info: dict) -> User:
        """Get or create a user from Authentik info."""
        from sqlalchemy import select

        provider_id = str(user_info["sub"])
        stmt = select(User).where(User.provider == "authentik", User.provider_id == provider_id)
