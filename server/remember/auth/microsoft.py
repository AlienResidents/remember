...
class MicrosoftAuthProvider(AuthProvider):
    ...
    async def _exchange_code(self, code: str) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{self._base_url}/oauth2/v2.0/token",
                data={
    ...
    async def _get_or_create_user(self, db, user_info: dict) -> User:
        """Get or create a user from Microsoft info."""
        from sqlalchemy import select

        provider_id = str(user_info["id"])
        stmt = select(User).where(User.provider == "microsoft", User.provider_id == provider_id)
